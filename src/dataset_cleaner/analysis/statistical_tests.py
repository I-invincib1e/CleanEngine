...
class StatisticalTester:
    ...
    def test_normality(self, columns=None, alpha=0.05):
        ...
        for col in columns:
            ...
            try:
                ...
                ks_stat, ks_p = stats.kstest(data, lambda x: stats.norm.cdf(x, data.mean(), data.std()))
            ...
            consensus_normal = normal_votes > total_tests / 2 if total_tests > 0 else False
            ...
            if not consensus_normal:
                self.insights.append(f"{col} does not follow normal distribution (consider transformation)")
            elif results['consensus']['confidence'] < 0.7:
                self.insights.append(f"{col} normality is uncertain - use non-parametric tests")
    ...
    def anova_test(self, dependent_var, independent_var, alpha=0.05):
        ...
        if dependent_var not in self.df.columns or independent_var not in self.df.columns:
            return None
        ...
        # Group data
        groups = [group[dependent_var].values for name, group in clean_data.groupby(independent_var)]
    ...
    def chi_square_test(self, var1, var2, alpha=0.05):
        ...
        try:
            ...
            chi2_results['chi_square'] = {
                ...
                'assumption_met': cells_below_5 / total_cells < 0.2 and min_expected >= 1
            }
            ...
            if p_value < alpha:
                ...
                self.insights.append(f"Significant association between {var1} and {var2} (Cramér's V = {cramers_v:.3f}, {effect_size} effect)")
    ...
    def correlation_tests(self, var1, var2, alpha=0.05):
        ...
        try:
            kendall_tau, kendall_p = stats.kendalltau(clean_data[var1], clean_data[var2])
        ...
        for method, result in correlation_results.items():
            if result.get('significant', False):
                ...
                strength = 'strong' if corr_strength > 0.7 else 'moderate' if corr_strength > 0.3 else 'weak'
                direction = 'positive' if result['correlation'] > 0 else 'negative'
                self.insights.append(f"{strength.title()} {direction} {method} correlation between {var1} and {var2} (r = {result['correlation']:.3f})")
            ...
            if len(groups) == 2:
                ...
                if len(group1) >= 2 and len(group2) >= 2:
                    try:
                        ...
                        t_stat_equal, p_value_equal = stats.ttest_ind(group1, group2, equal_var=True)
                        
                        # Unequal variances t-test (Welch's)
                        t_stat_unequal, p_value_unequal = stats.ttest_ind(group1, group2, equal_var=False)
                        ...
                        variance_test = self.test_equal_variances([group1, group2], alpha)
                        if variance_test:
                            t_test_results['variance_test'] = variance_test
                        
                        # Generate insights
                        p_val = p_value_equal if variance_test and variance_test.get('levene', {}).get('equal_variances', True) else p_value_unequal
                        if p_val < alpha:
                            effect_size = 'large' if abs(cohens_d) > 0.8 else 'medium' if abs(cohens_d) > 0.5 else 'small'
        ...
        if var2 is not None:
            ...
            if len(clean_data) >= 3:
                try:
                    ...
                    # Generate insights
                    if p_value < alpha:
                        median_diff = clean_data[var1].median() - clean_data[var2].median()
            ...
            clean_data = self.df[[var1, group_var]].dropna()
            groups = [group[var1].values for name, group in clean_data.groupby(group_var)]
