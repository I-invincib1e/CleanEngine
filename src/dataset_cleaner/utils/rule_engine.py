...
class RuleEngine:
    def __init__(self, rules: Optional[List[Dict[str, Any]]] = None):
        self.rules: List[Rule] = [Rule(type=r.get('type',''), params=r.get('params',{})) for r in (rules or [])]
            ...
            try:
                if rtype == 'column_exists':
                    ...
                    if col not in df.columns:
                        violations.append({'rule_index': idx,'type': rtype,'column': col,'reason': f"Column '{col}' not found"})
                    else:
                        actual = str(df[col].dtype)
                        if allowed and actual not in allowed:
                            violations.append({'rule_index': idx,'type': rtype,'column': col,'reason': f"dtype '{actual}' not in {sorted(allowed)}"})
                    ...
                    if col not in df.columns:
                        ...
                        if pct > threshold:
                            violations.append({'rule_index': idx,'type': rtype,'column': col,'reason': f"Missing {pct:.1f}% exceeds {threshold:.1f}%"})
                    ...
                    if col not in df.columns:
                        ...
                        if below > 0 or above > 0:
                            violations.append({'rule_index': idx,'type': rtype,'column': col,'reason': f"{below} values < {vmin}, {above} values > {vmax}"})
                    ...
                    if col not in df.columns:
                        ...
                        invalid = sorted([v for v in uniques if v not in set(map(str, allowed))])
                        if invalid:
                            violations.append({'rule_index': idx,'type': rtype,'column': col,'reason': f"Invalid values present: {invalid}"})
                    ...
                    if pct > threshold:
                        violations.append({'rule_index': idx,'type': rtype,'reason': f"Duplicate rows {pct:.1f}% exceeds {threshold:.1f}%"})

                else:
                    # Unknown rule types are ignored but recorded
                    violations.append({'rule_index': idx,'type': rtype,'reason': 'Unknown rule type'})
            except Exception as e:
                violations.append({'rule_index': idx,'type': rtype,'reason': f"Rule evaluation error: {e}"})
