...
class TimeSeriesAnalyzer:
    ...
    def _detect_date_column(self):
        ...
        if date_candidates:
            ...
            date_candidates.sort(key=lambda x: {'datetime': 0, 'parseable': 1, 'name_based': 2}[x[1]])
    ...
    def _prepare_time_series(self):
        ...
        self.analysis_results['time_series_info'] = {
            ...
            'date_range': {
                ...
                'duration_days': (self.df[self.date_column].max() - self.df[self.date_column].min()).days
            },
    ...
    def analyze_trends(self):
        ...
        for col in self.value_columns:
            ...
            if p_value < 0.05 and abs(r_value) > 0.5:
                self.insights.append(f"{col} shows a {trend_direction} trend (R² = {r_value**2:.3f})")
    ...
    def analyze_seasonality(self):
        ...
        for col in self.value_columns:
            ...
            dow_groups = [group[col].values for name, group in clean_data.groupby('day_of_week') if len(group) > 1]
            ...
            if monthly_p and monthly_p < 0.05:
                self.insights.append(f"{col} shows significant monthly seasonality (p = {monthly_p:.4f})")
            if dow_p and dow_p < 0.05:
                self.insights.append(f"{col} shows significant weekly seasonality (p = {dow_p:.4f})")
    ...
    def analyze_stationarity(self):
        ...
        for col in self.value_columns:
            ...
            if not is_stationary:
                self.insights.append(f"{col} appears to be non-stationary (may need differencing)")
    ...
    def detect_anomalies_temporal(self):
        ...
        for col in self.value_columns:
            ...
            anomalies[col] = {
                ...
                'anomaly_dates': clean_data.loc[anomaly_mask, self.date_column].tolist(),
                'anomaly_values': clean_data.loc[anomaly_mask, col].tolist()
            }
            
            # Generate insights
            if anomaly_count > 0:
                self.insights.append(f"{col} has {anomaly_count} temporal anomalies ({(anomaly_count/len(clean_data)*100):.1f}%)")
            ...
            for col2 in self.value_columns[i+1:]:
                ...
                if abs(strongest_corr) > 0.7:
                    if abs(current_corr) == abs(strongest_corr):
                        self.insights.append(f"Strong current correlation between {col1} and {col2} ({strongest_corr:.3f})")
                    else:
                        best_lag = max(lagged_corrs.items(), key=lambda x: abs(x[1]))[0] if lagged_corrs else 'unknown'
                        self.insights.append(f"Strong lagged correlation between {col1} and {col2} at {best_lag} ({strongest_corr:.3f})")
    ...
    def generate_time_series_insights(self):
        ...
        if not self.date_column:
            self.insights.append("No date column detected - time series analysis limited")
            return
        ...
        if ts_info:
            ...
            self.insights.append(f"Time series spans {duration} days with {data_points} data points ({frequency} frequency)")
            
            if duration < 30:
                self.insights.append("Short time series - consider collecting more historical data")
            elif duration > 365:
                self.insights.append("Long time series available - good for seasonal analysis")
        
        # Frequency insights
        if ts_info.get('frequency') == 'irregular':
            self.insights.append("Irregular time intervals detected - consider data resampling")
    ...
    def _create_trend_plots(self, viz_folder):
        ...
        for i, (col, trend_info) in enumerate(trends.items()):
            ...
            # Plot data points
            axes[i].scatter(clean_data[self.date_column], clean_data[col], alpha=0.6, s=20)
            
            # Plot trend line
            slope = trend_info['slope']
            intercept = x_numeric.iloc[0] * slope  # Adjust intercept for plotting
            trend_line = slope * x_numeric + (clean_data[col].iloc[0] - slope * x_numeric.iloc[0])
            axes[i].plot(clean_data[self.date_column], trend_line, 'r-', linewidth=2, 
    ...
    def _create_anomaly_plots(self, viz_folder):
        ...
        for col, anomaly_info in anomalies.items():
            ...
            # Plot normal data
            plt.plot(clean_data[self.date_column], clean_data[col], 'b-', alpha=0.7, label='Normal')
            ...
            plt.scatter(anomaly_dates, anomaly_values, color='red', s=50, 
                       label=f'Anomalies ({len(anomaly_values)})', zorder=5)
            ...
            plt.tight_layout()
            plt.savefig(viz_folder / f'anomalies_{col}.png', dpi=300, bbox_inches='tight')
            plt.close()